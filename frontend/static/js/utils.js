    ...
    updateNavAuth() {
        ...
        if (API.isLoggedIn()) {
            ...
            navAuth.innerHTML = `
                ...
                <div class="btn-group">
                    ${isAdmin ? `
                        <a href="/control-panel/" class="btn btn-accent btn-sm">Admin Panel</a>
                    ` : ''}
                    <button class="btn btn-outline btn-sm" onclick="API.logout()">Logout</button>
                </div>
            `;

            // Hide user-only links for admins
            if (isAdmin) {
                document.querySelectorAll('.user-only-link').forEach(link => {
    ...
    buildStationCard(station, userLocation = null) {
        ...
        return `
            <div class="station-card status-${status}" data-station-id="${station.id}"
                ...
                <div class="station-actions" style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px;">
                    <button class="btn btn-primary btn-sm" onclick="event.stopPropagation(); SmartMap.showDirections(${station.latitude}, ${station.longitude}, '${station.name.replace(/'/g, "\\'")}')" style="flex: 1;">
                        🧭 Directions
                    </button>
                    ${!isAdmin ? `
                    <button class="btn btn-outline btn-sm" onclick="event.stopPropagation(); BookingUI.openBookingModal(${station.id}, '${station.name.replace(/'/g, "\\'")}')" style="flex: 1;">
                        📅 Book Slot
                    </button>
                    <button class="btn btn-outline btn-sm" onclick="event.stopPropagation(); window.location.href='/station/${station.id}/'" style="flex: 1;">
                        ℹ Details
                    </button>
                    ` : `
                    <button class="btn btn-outline btn-sm" onclick="event.stopPropagation(); window.location.href='/station/${station.id}/'" style="flex: 1;">
